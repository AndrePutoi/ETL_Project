...
def enriquecer_dados_com_ano(df, coluna_ano="ano"):
    ...
    # Década (ex: 2010)
    df["decada"] = (df[coluna_ano] // 10) * 10

    # Século (ex: 21)
    df["seculo"] = df[coluna_ano].apply(lambda x: (x - 1) // 100 + 1)

    # Bissexto
    df["bissexto"] = df[coluna_ano].apply(calendar.isleap)

    # Posição na década (0 a 9)
    df["pos_decada"] = df[coluna_ano] % 10
    return df
